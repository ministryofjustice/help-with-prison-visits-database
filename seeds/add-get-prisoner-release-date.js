require('dotenv').config()

/**
 * Adds a table function to the IntSchema that retrieves the last approved Claim Child data for the given reference
 * number and eligibilityId or by claimId and grants the external web user permissions to call it.
 */
exports.seed = function (knex, Promise) {
  return knex.schema
    .raw('DROP FUNCTION IF EXISTS getPrisonerReleaseDate')
    .then(function () {
      return knex.schema
        .raw(
          `
          CREATE FUNCTION IntSchema.getPrisonerReleaseDate(@eligibilityId int)
          RETURNS TABLE
          AS
          RETURN
          (
            SELECT TOP 1 Prisoner.ReleaseDateIsSet, Prisoner.ReleaseDate           
            FROM IntSchema.Eligibility AS Eligibility
              JOIN IntSchema.Prisoner AS Prisoner ON Prisoner.EligibilityId = Eligibility.EligibilityId
            WHERE
              Prisoner.EligibilityId = @eligibilityId
          )
          `
        )
        .raw('GRANT SELECT ON getPrisonerReleaseDate TO ??;', [process.env.HWPV_EXT_WEB_USERNAME])
    })
    .catch(function (error) {
      console.log(error)
      throw error
    })
}
